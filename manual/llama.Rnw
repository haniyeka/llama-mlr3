...
are available through the mlr3 package~\cite{mlr}.
...
LLAMA uses the \lstinline{mlr3} package~\cite{mlr} as an interface to R's many
...
LLAMA to use them than the vanilla functions. Familiarity with \lstinline{mlr3}
...
<<eval=FALSE>>=
data = input(read.csv("features.csv"), read.csv("times.csv"), successes=read.csv("success.csv"))
@
...
offers, at the time of writing, 29 different scenarios that have been used in
...
folds = cvFolds(ldf)
model = classify(lrn("classif.J48"), folds)
...
\lstinline{mlr3} documentation~\cite{mlr}.
...
<<warning=FALSE>>=
model = regression(lrn("regr.lm"), folds)
@
...
<<>>=
model = classifyPairs(lrn("classif.J48"), folds)
@
...
<<>>=
model = cluster(lrn("clust.xmeans"), folds)
@
...
<<>>=
model = cluster(lrn("clust.xmeans"), folds, pre=normalize)
mean(misclassificationPenalties(folds, model))
mean(parscores(folds, model))
sum(successes(folds, model))
@
...
<<warning=FALSE>>=
imputed = imputeCensored(data, lrn("regr.lm"))
imputedFolds = cvFolds(imputed)
imputedModel = regression(lrn("regr.lm"), imputedFolds)
mean(misclassificationPenalties(folds, model))
mean(parscores(folds, model))
sum(successes(folds, model))
...
ensembleModel = classify(list(lrn("classif.J48"),
        lrn("classif.rpart"),
        lrn("classif.kknn")),
    folds)
stackedEnsembleModel = classify(list(lrn("classif.J48"),
        lrn("classif.rpart"),
        lrn("classif.kknn"),
        .combine=lrn("classif.J48")),
    folds)
...
<<warning=FALSE>>=
stackedModel = regression(lrn("regr.lm"), folds,
    combine=lrn("classif.J48"))
mean(misclassificationPenalties(folds, model))
mean(parscores(folds, model))
sum(successes(folds, model))
...
LLAMA does support an additional argument for \lstinline{regression} that makes  this task easier. The \lstinline{expand} argument allows to specify a
...
<<warning=FALSE>>=
stackedExpandedModel = regression(lrn("regr.lm"), folds,
    combine=lrn("classif.J48"),
    expand=function(x) {
...
While the function given here may appear cryptic at first, it demonstrates one of
...
model1 = classify(lrn("classif.J48"), folds)
model2 = classify(lrn("classif.randomForest"), folds)
...
folds = cvFolds(satsolvers)
model1 = classify(lrn("classif.J48"), folds)
model2 = regression(lrn("regr.lm"), folds)
model3 = classifyPairs(lrn("classif.J48"), folds)
model4 = regressionPairs(lrn("regr.lm"), folds)
model5 = cluster(lrn("clust.xmeans"), folds)
...
satisfiableTask = as_task_classif("satisfiable", target="satisfiable", data=cbind(satisfiable, subset(featureData, T, featureNames)))
satisfiableModel = train(lrn("classif.J48"), task=satisfiableTask)
...
featureTimeTask = as_task_regr("featureTime", target="time", data=cbind(featureTimes, subset(reducedFeatureData, T, reducedFeatureNames)))
featureTimeModel = train(lrn("regr.lm"), task=featureTimeTask)
featureTimeLimit = 5
data = merge(subset(read.csv("features.csv"), featureTimes$time < featureTimeLimit), read.csv("performance.csv"))
@
...
satisfiableModel = classifyPairs(lrn("classif.randomForest"), satisfiableFolds)
unsatisfiableModel = classifyPairs(lrn("classif.randomForest"), unsatisfiableFolds)
@
...
myPortfolioSolver = function(instance) {
    ...
    if(!isSolved(result)) {
        ...
        featureTime = predict(featureTimeModel, newdata=simpleFeatures)$data$response
