...
function(clusterer=NULL, data=NULL, bestBy="performance", pre=function(x, y=NULL) { list(features=x) }, save.models=NA) {
    ...
    predictions = rbind.fill(parallelMap(function(i) {
      ...
      for(j in 1:length(clusterer)) {
        task = TaskClust$new(id="clustering", backend = trf$features)
        ...
        trainclusters = model$predict_newdata(newdata=trf$features)$partition
        ...
        if(inherits(combinator, "Learner")) { # only do this if we need it
          trainpredictions[,j] = factor(sapply(model$predict_newdata(newdata=trf$features)$partition,
                                               function(x) {
                                                 if(is.na(x)) {
                                                   factor(NA)
                                                 } else {
        ...
        preds = model$predict_newdata(newdata=tsf$features)$partition
    ...
    models = lapply(1:length(clusterer), function(i) {
      ...
      clusters = model$predict_newdata(newdata=fs$features)$partition
      best = by(data$data, clusters, bestfun)
      return(function(newdata) {
        preds = model$predict_newdata(newdata=newdata)$partition
